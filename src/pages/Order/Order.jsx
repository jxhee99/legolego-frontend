...
const Order = () => {
  ...
  const onClickBuy = async () => {
    ...
    try {
      ...
      if (orderResponse.status === 201) {
        ...
        navigate('/payment', {
          state: { product, count, totalPrice, orderData, merchantUid },
        });
      } else {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.order_box}>
        <div className={styles.order_info}>
          ...
          <div className={styles.quantity}>
            <button className={styles.quantity_btn} onClick={onClickMinus}>
              -
            </button>
            <span className={styles.quantity_span}>{count}</span>
            <button className={styles.quantity_btn} onClick={onClickPlus}>
              +
            </button>
          </div>
          <span className={styles.total_price}>
            결제금액 : {(count * product.price).toLocaleString()}
      ...
      <button className={styles.buy_btn} onClick={onClickBuy}>
        BUY NOW
      </button>
      <hr className={styles.hr2} />
    </div>
