...
const Notification = ({ role }) => {
    ...
    useEffect(() => {
        ...
        if (userNum && accesstoken) { // token 조건 추가
            console.log(`Connecting to WebSocket for user ${userNum}`);
            websocketService.connect(userNum, accesstoken, (message) => { // token 추가
                console.log('New message: ', message);
                setNotifications((prevNotifications) => [...prevNotifications, message]);
                setHasNewNotification(true); // 새로운 알림이 도착하면 상태를 업데이트
                console.log('setHasNewNotification', true);
    ...
    const fetchNotifications = async () => {
        ...
        try {
            ...
            setNotifications(response.data);
            // 새로운 알림이 존재하는지 확인
            const hasNew = response.data.some(notification => !notification.isChecked);
